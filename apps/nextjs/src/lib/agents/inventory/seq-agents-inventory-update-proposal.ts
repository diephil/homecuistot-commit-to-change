...
export async function createInventoryManagerAgentProposal(
  ...
  try {
      ...
      if (event.content?.parts) {
        for (const part of event.content.parts) {
          ...
          if ("functionCall" in part && part.functionCall) {
            // End model span before starting tool span
            if (currentModelSpan) {
    ...
    traceCtx.trace.update({
      output: {
        recognizedCount: proposal?.recognized.length ?? 0,
        unrecognizedCount: proposal?.unrecognized.length ?? 0,
      },
    });
    traceCtx.end();
