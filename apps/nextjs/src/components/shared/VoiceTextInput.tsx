  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        {/* Mode toggle - always at bottom */}
        {!permissionDenied && !showRecordingUI && !processing && (
          <button
            onClick={() => setInputMode(effectiveInputMode === "voice" ? "text" : "voice")}
            className="text-sm text-gray-600 hover:text-gray-900 underline cursor-pointer disabled:cursor-not-allowed"
            disabled={isDisabled}
            aria-label={effectiveInputMode === "voice" ? "Switch to text input" : "Switch to voice input"}
          >
            {effectiveInputMode === "voice" ? textFallbackLabel : "Use voice instead?"}
          </button>
        )}
      </div>
    </div>
