...
async function main() {
  ...
  for (const filePath of promptFiles) {
    try {
      ...
      if (!importedModule.PROMPT) {
        console.warn(
          `⚠️  Skipped ${path.basename(path.dirname(filePath))}: No PROMPT export`,
        );
        continue;
      }

      const { PROMPT } = importedModule;
      ...
      console.error(
        `❌ ${agentName}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
