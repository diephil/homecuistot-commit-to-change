...
- [ ] T005 Implement `isNewUser` service in `apps/nextjs/src/lib/services/brand-new-user.ts` — export `async function isNewUser(params: {db: DrizzleDB}): Promise<boolean>` that counts `user_inventory` rows and `user_recipes` rows, returns `true` if both are 0. Uses parameterized Drizzle queries (same pattern as `getUserCounts()` in `apps/nextjs/src/app/actions/cooking-log.ts` but accepts a `db` param for reuse in API routes)
